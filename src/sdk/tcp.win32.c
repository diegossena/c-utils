...
SDK_EXPORT void tcp_connect(tcp_t* this, net_address_t* address) {
  this->__socket = __socket_new(address->family, SOCKET_TYPE_STREAM);
  if (this->__socket < 0) {
    closesocket(this->__socket);
  }
...
SDK_EXPORT void __tcp_connect_handle(tcp_t* this) {
  ...
  this->__task.handle = (callback_t)__tcp_connecting_handle;
...
SDK_EXPORT void __tcp_connecting_handle(tcp_t* this) {
  ...
  if (error_code > 0) {
    ...
    this->onconnect(this);
    if (this->__task.handle == (callback_t)tcp_free) {
      tcp_free(this);
    } else {
      this->__updated_at = date_now();
    }
