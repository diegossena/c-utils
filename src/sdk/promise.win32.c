...
SDK_EXPORT void _promise_run() {
  ...
  result = GetQueuedCompletionStatus(__sdk_iocp, &bytes, (PULONG_PTR)&task, &overlapped, 0);
  if (result) {
    result = 0;
  } else {
    result = GetLastError();
